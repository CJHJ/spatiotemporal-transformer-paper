...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        _, (hidden_output, cell_output) = self.lstm(x, (h_n, c_n))
...
class Decoder(nn.Module):
    ...
    def forward(self, x, hidden_encoder, cell_encoder):
...
class LSTMSeq2Seq(nn.Module):
    def __init__(
        ...
        self.linear_out = nn.Linear(
            lstm_hidden_size,
            feature_size * (1 if not quantiles else self.n_quantiles),
        )
    ...
    def one_step_ahead_forward(
        ...
        hidden_encoder, cell_encoder = self.encoder(input_batch)

        target_batch = F.gelu(self.linear_in(target_batch))
        decoder_output = self.decoder(
            target_batch, hidden_encoder, cell_encoder
        )
